...
5. **Positive `avg_multiplier` required**: Players with `avg_multiplier <= 0` are automatically filtered out (division by zero). The UI shows a warning with the count of filtered rows, and the simulation runs on the remaining valid players. The PRD does not specify how to handle this.
